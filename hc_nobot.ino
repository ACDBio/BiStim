...
static float g_target_amplitude_ua = 300.0f; // Желаемая амплитуда тока в МИКРОамперах (1000 мкА = 1 мА)
...
int   g_baseline_duties[5]   = {0, 60, 120, 180, 240};
...
float getAbsoluteCurrent_mA(float raw_mv) {
    float delta_mv = INA_REF_MV - raw_mv; 
...
// float getAbsoluteCurrent_mA(float raw_mv) {
//     // Твой изначальный fabsf спасет растущий сигнал
//     float delta_mv = fabsf(raw_mv - g_isense_zero_mv); 
    
//     if (delta_mv < 0.0f) delta_mv = 0.0f; // Это здесь уже не сработает, сигнал выживет
    
//     return delta_mv / (ISENSE_SHUNT_OHMS * ISENSE_INA_GAIN);
// }
// float getAbsoluteCurrent_mA(float raw_mv) {
//     // Берем разницу с твоим откалиброванным нулем и по модулю (fabsf), 
//     // чтобы ловить амплитуду в любую сторону (и рост, и падение)
//     float delta_mv = fabsf(g_isense_zero_mv - raw_mv); 
    
//     // Защита от совсем мелкого шума АЦП
//     if (delta_mv < 0.5f) delta_mv = 0.0f; 
...
// Переименовано из currentFromBot_mA, теперь принимает raw милливольты напрямую
float currentFromRaw_mA(float active_mv) {
    float total_bat_mA = getAbsoluteCurrent_mA(active_mv);

    // Бэйзлайн вычитаем только если он готов
    float dynamic_baseline_mA = g_baseline_ready ? getDynamicBaseline_mA(g_last_duty) : 0.0f;
...
void updateTestSignal(float dt_s) {
#if TEST_ENABLED
    // 1) фаза (используем реальный dt, а не предполагаемую 1 мс)
    float delta_phase = 2.0f * M_PI * g_stim_freq_hz * dt_s;
    g_stim_phase += delta_phase;
    while (g_stim_phase > (2.0f * M_PI)) g_stim_phase -= (2.0f * M_PI);
    ...
    // 3.1) СБРОС ИНТЕГРАЛА ЗАКОММЕНТИРОВАН — теперь ПИД не обнуляется при переходе через ноль
    ...
    if (fabsf(target_mA) < DEADZONE_MA) {
        ledcWrite(PIN_DRV_IN1, 0);
        ledcWrite(PIN_DRV_IN2, 0);
        // g_integral = 0.0f; // ЗАКОММЕНТИРОВАНО — пусть ПИД помнит усилие даже в нуле
    ...
    // 5) измерение (ТЕПЕРЬ НАПРЯМУЮ RAW, КАК ТЫ И ПРОСИЛА)
    float raw_ina_mv = readIsenseAvg_mV();

    float abs_measured_mA = currentFromRaw_mA(raw_ina_mv);   
    ...
    // (ШИМ и интеграл больше НЕ блокируются, ПИД сможет продавить 20кОм)
    if (abs_measured_mA < NO_CURRENT_THRESHOLD_MA && abs_target_mA > 0.1f) {
        ...
        if (g_low_current_count > NO_CURRENT_TIMEOUT_MS) {
            ...
            // if (duty > MAX_DUTY_WHEN_NO_CURRENT) duty = MAX_DUTY_WHEN_NO_CURRENT; // ЗАКОММЕНТИРОВАНО
            // g_integral = 0.0f; // ЗАКОММЕНТИРОВАНО
        }
    } else {
...
static float g_hc_baselines[5] = { 0.00f, 0.50f, 1.20f, 2.50f, 3.20f }; 
...
static float g_saved_amplitude_ua = 0.0f;
...
// Вызывать в конце setup()
void loadHardcodeCal() {
#if HARDCODE_CAL
    for(int i = 0; i < 5; i++) {
        g_baseline_currents[i] = g_hc_baselines[i];
    }
    g_baseline_current_mA = g_hc_baselines[1]; 
    g_baseline_ready      = true;
    g_cal_valid           = true;
    Serial.println(">>> HARDCODE CAL loaded: Таблица токов холостого хода применена.");
#endif
}
...
void calibrationStep() {
    switch (g_cal_state) {
        case CAL_BASELINE:
            g_cal_current_sum += getAbsoluteCurrent_mA(log_shunt_mv); 
            g_cal_duty_sum    += log_duty;
            g_cal_count++;
            break;
        case CAL_KNOWN_RES:
            g_cal_current_sum += currentFromRaw_mA(log_shunt_mv); 
            g_cal_duty_sum    += log_duty;
            g_cal_count++;
            break;
        default: break;
...
void checkStep() {
    ...
    g_check_current_sum += currentFromRaw_mA(log_shunt_mv); 
        ...
        case 'c': { // старт baseline — прогон по массиву ШИМ без нагрузки
            ...
            for (int i = 0; i < 5; i++) {
                ...
                for (int j = 0; j < 200; j++) {
                    ...
                    sum_mA += getAbsoluteCurrent_mA(raw_mv);
                    count++;
                    delay(5); 
                }
...
void loop() {
    ...
    while (Serial.available()) {
        ...
        uint32_t now = micros();
        uint32_t dt_us = now - lastStimMicros;
        if (dt_us >= 1000) {
            lastStimMicros = now;
            float dt_s = (float)dt_us * 1e-6f;   // реальный шаг в секундах
            updateTestSignal(dt_s);
            calibrationStep();
            checkStep();
        }
    #endif
    ...
    if (g_log_enabled && nowMs - lastPrintMs >= 5) {
        lastPrintMs = nowMs;

        Serial.printf("Shunt_Raw_mV:%.1f,Meas_mA:%.3f,Exp_mA:%.3f,Duty:%d,"
